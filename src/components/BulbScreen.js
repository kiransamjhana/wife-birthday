...
const BulbScreen = () => {
  ...
  const [messages, setMessages] = useState([
    "Happy BirthDay samju. You are always very close to my heart. Your Birthday brings alot of joy and hapiness in me. Wish you very Happy Birthday my love ",
  ]);
    ...
    <div className={`bulb-container ${showButton ? "show-button" : ""}`}>
      {bulbImages.map((imageName, index) => (
        <LazyLoad
          ...
          height={100}
          offset={[-100, 100]}
          placeholder={<div className="bulb" />}
        >
          <img
            className={`bulb ${index === currentBulbIndex ? "highlight" : ""}`}
            src={`assets/${
              index === currentBulbIndex ? "bulb_white.png" : imageName
            }`}
            alt="Bulb"
          />
        </LazyLoad>
      ...
      {showBalloonsButton && showTempButton && (
        <div className="button-container">
          <button className="button" onClick={handleFlySingleBalloon}>
            Let's fly our Samju best ballon
          </button>
        </div>
      )}
      {showSingleBalloon && (
          ...
          {roamingBalloons.map((balloon) => (
            <img
              ...
              style={{
                ...
                bottom: balloon.position.bottom,
              }}
            />
          ))}
        </div>
      ...
      {showDeliciousCakeButton && (
        <div className="button-container">
          <button className="button" onClick={handleDeliciousCake}>
            Here you go your Delicious Cake
          </button>
        </div>
      )}
      {showLightCandlesButton && (
        <div className="button-container">
          <button className="button" onClick={handleLightCandle}>
            Alight! Lets Light up Candle for samju
          </button>
        </div>
      )}
      {showMessageButton && (
        <div
          className={`message-container ${
            isShowingMessage ? "show-message" : ""
          }`}
        >
          {messages &&
            messages.length > 0 &&
            messages[currentMessageIndex] !== undefined &&
